...
// Check-out an article from specific storage-room id
router.post('/check-out', (req, res) => {
  const checkoutArticle = {
    material_number: req.body.material_number,
    comment: req.body.comment,
    storage_room: req.body.storage_room,
  };
  if (!checkoutArticle.material_number || !checkoutArticle.storage_room) {
    res.status(400).send('Bad request');
  } else {
    // eslint-disable-next-line consistent-return
    let sql1 = 'INSERT INTO StorageEvent (action, timestamp, user, comment, package, shelf, storage_room, branch, article)';
    sql1 += ' SELECT "checked-out", 55555, 1, ?, Package.package_number, Shelf.shelf_name, StorageRoom.name as "storageroom", Branch.name, Article.id FROM Package, Shelf, StorageRoom, Branch, Article WHERE';
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(sql1, [checkoutArticle.comment, checkoutArticle.material_number, checkoutArticle.material_number, checkoutArticle.material_number, checkoutArticle.storage_room, checkoutArticle.storage_room, checkoutArticle.material_number], (err1, result1) => {
        connection.release();
        if (err1) {
          console.log(err1);
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(sql2, [checkoutArticle.material_number], (err, result) => {
        connection.release();
        if (err) {
          console.log(err);
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(sql3, [checkoutArticle.comment, checkoutArticle.material_number, checkoutArticle.material_number, checkoutArticle.material_number, checkoutArticle.storage_room, checkoutArticle.storage_room, checkoutArticle.material_number], (err, result) => {
        connection.release();
        if (err) {
          console.log(err);
